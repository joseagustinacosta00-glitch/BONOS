...
class MarketHistoryStorage:
    ...
    async def cleanup_outside_window(
        self,
        *,
        future_open_minutes: int,
        future_close_minutes: int,
        other_open_minutes: int,
        other_close_minutes: int,
        non_business_days: Sequence[date] | None = None,
    ) -> dict[str, int]:
        """Borra ticks fuera de la ventana correspondiente a su categoria.
        - Categoria 'future': mantiene solo [future_open_minutes, future_close_minutes].
        - Resto: mantiene solo [other_open_minutes, other_close_minutes].
        El minuto se calcula como hour*100 + minute en zona AR.
        Tambien borra todos los ticks de las fechas listadas en non_business_days.
        DESTRUCTIVO. Devuelve stats por paso."""
        stats = {"deleted_non_business_day": 0, "deleted_outside_window": 0}
        if self._pool is None:
            return stats
        async with self._pool.acquire() as connection:
            if non_business_days:
                rows = await connection.execute(
                    ...
                    WHERE (ts AT TIME ZONE 'America/Argentina/Buenos_Aires')::date = ANY($1::date[])
                    """,
                    list(non_business_days),
                )
                try:
                    stats["deleted_non_business_day"] = int(str(rows).split()[-1])
                except Exception:
                    pass
            rows = await connection.execute(
                """
                DELETE FROM market_ticks t
                USING instruments i
                WHERE t.instrument_id = i.id
                  AND (
                    (i.category = 'future' AND (
                       (extract(hour FROM t.ts AT TIME ZONE 'America/Argentina/Buenos_Aires') * 100 +
                        extract(minute FROM t.ts AT TIME ZONE 'America/Argentina/Buenos_Aires')) < $1
                       OR
                       (extract(hour FROM t.ts AT TIME ZONE 'America/Argentina/Buenos_Aires') * 100 +
                        extract(minute FROM t.ts AT TIME ZONE 'America/Argentina/Buenos_Aires')) > $2
                    ))
                    OR
                    (i.category <> 'future' AND (
                       (extract(hour FROM t.ts AT TIME ZONE 'America/Argentina/Buenos_Aires') * 100 +
                        extract(minute FROM t.ts AT TIME ZONE 'America/Argentina/Buenos_Aires')) < $3
                       OR
                       (extract(hour FROM t.ts AT TIME ZONE 'America/Argentina/Buenos_Aires') * 100 +
                        extract(minute FROM t.ts AT TIME ZONE 'America/Argentina/Buenos_Aires')) > $4
                    ))
                  )
                """,
                future_open_minutes,
                future_close_minutes,
                other_open_minutes,
                other_close_minutes,
            )
            try:
                stats["deleted_outside_window"] = int(str(rows).split()[-1])
            except Exception:
                pass
        return stats

    async def list_distinct_dates_ar(self) -> list[date]:
        """Devuelve las fechas distintas (en TZ AR) presentes en market_ticks."""
        if self._pool is None:
            return []
        async with self._pool.acquire() as connection:
            rows = await connection.fetch(
                """
                SELECT DISTINCT (ts AT TIME ZONE 'America/Argentina/Buenos_Aires')::date AS d
                FROM market_ticks
                """
            )
